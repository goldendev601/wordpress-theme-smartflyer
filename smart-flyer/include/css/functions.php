...
{  	
	...
	wp_register_script('main', get_template_directory_uri().'/js/main.js', array('jquery'), '5.1', true);
	...
	wp_register_script('public-script', get_template_directory_uri().'/include/js/public-script.js', array('jquery'), '4.0', true);
...
{	
	...
	wp_enqueue_style('custom-style', get_stylesheet_directory_uri().'/include/css/custom-style.css', false, '10.10');
...
/************ Get More Contributions Ajax ***************/
add_action('wp_ajax_nopriv_smartflyer_contribution', 'ajax_smartflyer_contribution');
add_action('wp_ajax_smartflyer_contribution', 'ajax_smartflyer_contribution');
function ajax_smartflyer_contribution() {
	$args = array(
		'post_type' => 'review',
		'posts_per_page' => 2,
		'orderby' => 'date',
	...
	if($ajaxposts->have_posts()) {
		ob_start();
		while($ajaxposts->have_posts()) : $ajaxposts->the_post();
			get_template_part('_objects/agent', 'contribution');
		endwhile;
		$response .= ob_get_clean();
	} else {
		$response = '';
