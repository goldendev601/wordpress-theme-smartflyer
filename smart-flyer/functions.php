...
//require('_inc/cpt-industrynews.php');
require('_inc/cpt-industry-news.php');
...
{  	
	...
	wp_register_script('login-script', get_template_directory_uri().'/js/login.js', array('jquery'), time(), true);
	...
	wp_register_script('public-script', get_template_directory_uri().'/include/js/public-script.js', array('jquery'), '3.3', true);
	wp_enqueue_script('public-script');
	

	if(is_page('insurance-addon-information') || is_page('insurance-addon-tab') || is_page('experiences-addons-tab') || is_page('experiences-addons-information') || is_page('yacht-sea-transportation-tab') || is_page('yacht-sea-transportation-information') || is_page('helicopter-air-transportation-tab') || is_page('helicopter-air-transportation-information') || is_page('rental-car-information') || is_page('rental-car-tab') || is_page('on-site-tab') || is_page('on-site-information') || is_page('cruise-tab') || is_page('cruise-information') || is_page('villas-tab') || is_page('villas-information') || is_page('brands-aman-tab') || is_page('brands-aman-information') || is_page('hotel-mauna-lani-tab') || is_page('hotel-mauna-lani-information') || is_page('private-aviation-aero-information') || is_page('private-aviation-aero-tab')){
		
		wp_register_script('custom-form-information-script', get_template_directory_uri().'/include/js/custom-form-information-script.js', array('jquery'), '1.0', true);
		wp_enqueue_script('custom-form-information-script');
	}
};
...
{	
	...
	wp_enqueue_style('custom-style', get_stylesheet_directory_uri().'/include/css/custom-style.css', false, '9.0');
	...
	if(is_page('insurance-addon-tab') || is_page('insurance-addon-information') || is_page('experiences-addons-tab') || is_page('experiences-addons-informations') || is_page('yacht-sea-transportation-tab')|| is_page('yacht-sea-transportation-informations') || is_page('helicopter-air-transportation-tab') || is_page('helicopter-air-transportation-informations') || is_page('rental-car-form-tab') || is_page('rental-car-informations') || is_page('on-site-information') || is_page('on-site-tab') || is_page('cruise-form-tab') || is_page('cruise-information') || is_page('villas-form-tab') || is_page('villas-information') || is_page('brands-aman-tab') || is_page('brands-aman-informations') || is_page('hotel-mauna-lani-tab') || is_page('hotel-mauna-lani-informations') || is_page('private-aviation-airo-tabs') || is_page('private-aviation-airo-informations') || is_page('private-aviation-air-transportation-information') || is_page('private-aviation-air-transportation-tabs')){
		wp_enqueue_style('custom-form-information-style', get_stylesheet_directory_uri().'/include/css/custom-form-information-style.css', false, '1.0');
	}
...
add_action( 'init', 'smartflyer_postType_data_API_callback' );
function smartflyer_postType_data_API_callback(){

	if ( isset( $_GET['smtAPI'] ) && !empty( $_GET['smtAPI'] ) && $_GET['smtAPI'] == 'getAgentList' ) {
		...
		// New wp query
		$agent = new WP_Query( $args );
		
		$agentArray = [];
		$agentArray['total_count'] = $agent->found_posts;

		foreach ( $agent->posts as $key => $value ) {
			$permalink = get_permalink( $value->ID );
			$agent_email = get_post_meta( $value->ID, '_agent_email', true );
			$agent_website = get_post_meta( $value->ID, '_agent_website', true );
			$locations = get_the_terms( $value->ID, 'agent_location' );
			$post_image = get_the_post_thumbnail_url( $value->ID );

			$locArray = [];
			foreach ( $locations as $locKey => $locValue ) {
				$locArray[] = $locValue->name;
			}
			...
			$agentArray['records'][] = array( 
										'postName' => $value->post_title,
										'postLink' => $permalink,
		...
		echo json_encode( $agentArray );die();
	}
}
...
function wpse_376370( $value, $wp_query ) {
    if (
        ...
        ! $wp_query->is_page( 'plan-a-trip' ) &&
        // and the Page is not using the template some-template.php.
        ! is_page_template( 'page-plan-a-trip.php' )
    ) {
...
function plan_a_trip_page_parse_request( $wp ) {
    // custom variables will only be available on popular page.
    $pageslug = 'agent';

    global $wp;

    $wp->query_vars['agent'] = $pageslug;
...
//add_action( 'parse_request', 'plan_a_trip_page_parse_request', 0 );

add_filter( 'query_vars', 'addnew_query_vars', 10, 1 );
function addnew_query_vars($vars)
{   
    $vars[] = 'agent'; // c is the name of variable you want to add       
    return $vars;
}

function custom_rewrite_basic() 
{
    add_rewrite_rule('^plan-a-trip/([a-z]+)/?', 'plan-a-trip?agent=$1', 'top');
}
add_action('init', 'custom_rewrite_basic');

/* CRM Login */
add_action('admin_init', 'add_agent_role');
function add_agent_role() {  
    //Add the Agent user role
...
add_action( 'wp_ajax_check_login_callback', 'check_login_callback' );
add_action( 'wp_ajax_nopriv_check_login_callback', 'check_login_callback' );
function check_login_callback(){
	if ( !empty( $_POST['login'] ) ) {
		...
		if ( !empty( $getEmail ) && !empty( $getPassword ) ) {
			...
			$response = callAPI( 'POST', 'http://stagingsociety.smartflyer.com/api/auth/login', $params );

			if ( !empty( $response->access_token ) && !empty( $response->user ) ) {
				$message = create_custom_user( $response , $getPassword, $getEmail );
			}else{
			...
			if ( empty( $getPassword ) ) {
				$message .= '<p>Please Enter Password</p>';
			}
		}
	}elseif ( !empty( $_POST['logout'] ) ) {

		$accessToken = get_user_meta( get_current_user_id(), 'login_access_token', true );

		$response = callAPI( 'POST', 'http://stagingsociety.smartflyer.com/api/auth/logout', false, $accessToken );
		wp_logout();
		$message = array( 'redirectUrl' => site_url('crm-login'), 'redirect' => true );
	}
...
function create_custom_user( $response, $password, $email ) {

	if ( username_exists( $response->user->name ) || email_exists( $email ) ) {

		if ( !is_user_logged_in() ) {
			$user = get_userdatabylogin( $response->user->name );
	        $user_id = $user->ID;
	        wp_set_current_user( $user_id, $response->user->name );
	        wp_set_auth_cookie( $user_id );
    		
    		update_user_meta( $user_id, 'login_access_token', $response->access_token );
	        update_user_meta( $user_id, 'user_profile', $response->user->image_url );
	        ...
	        $data = array( 'redirectUrl' => site_url('insurance-addon-information'), 'redirect' => true );
	        return $data;
    	}else{
    ...
    if ( username_exists( $response->user->name ) == null && email_exists( $email ) == false ) {

        // Create the new user
        $user_id = wp_create_user( $response->user->name, $password, $email );
...
function change_user_avtar( $url, $id, $arg ) {
			
	$user_profile = get_user_meta( $id, 'user_profile', true );
...
add_filter( 'get_avatar_url', 'change_user_avtar', 10, 5 );